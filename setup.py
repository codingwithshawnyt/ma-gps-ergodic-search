...
def get_version() -> str:
    ...
    init = open(os.path.join("MAGPS", "__init__.py"), "r").read().split()
    return init[init.index("__version__") + 2][1:-1]


def get_install_requires() -> str:
    return [
        "gymnasium>=0.26.0,<0.29.0",
        "tqdm",
        "numpy>=1.22.0,<1.25.0",  # https://github.com/numpy/numpy/issues/12793
        "tensorboard>=2.5.0",
        "torch>=1.12.1",
        "numba>=0.51.0",
...
setup(
    name="MAGPS",
    version=get_version(),
    description="A Library for guided policy search, based on Tianshou (version 0.5.1)",
    url="https://github.com/jamesjingqili/ma-gps",
    author="james.jingqi.li, Gechen Qu",
    author_email="james.jingqi.li@gmail.com, GechenQu@gmail.com",
    license="MIT",
    python_requires=">=3.6",
    classifiers=[
